defmodule MCP.Test.ClientReviewHTTPPlug do
  ...
  def call(conn, opts) do
    ...
    cond do
      message["method"] == "server/discover" and Keyword.get(opts, :downgrade?) ->
        respond(conn, 400, %{
          "jsonrpc" => "2.0",
          "id" => message["id"],
          "error" => %{
      ...
      message["method"] == "tools/list" and Keyword.has_key?(opts, :recovery) ->
        recovery_tools_response(conn, message, opts)

      Keyword.get(opts, :bind_non_initialize?) ->
        conn
